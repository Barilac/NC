...
The file contains code for data analysis of the article.</br>
Stable isotope analysis in soil prospection reveals the type of historic land-use under contemporary temperate forests in Europe".</br>

**All data is available on the github repository.**

# Figure 1

Data loading, all data available at https://github.com/Barilac/SR in S1.txt.</br>


```{r figure1, echo=T, message=F, warning=F}
raw = read.table(file = "S1.txt", header=TRUE,
sep="\t", dec=".", check.names = F, stringsAsFactors = T)
...
```{r figure1_2, echo=T, message=F, warning=F}
...
countries <- subset(world, region %in% c("Czech Republic", "Peru","Slovakia","Bulgaria",
"Hungary", "Germany","Greece",
"USA","Japan", "Kenya", "UK","Denmark"))
...
p2 
```
```{r figure1_final, echo=T, message=F}
...
```{r figure4and5, echo=T, message=F}
raw = read.table(file = "S1.txt", header=TRUE,
sep="\t", dec=".", check.names = F,
stringsAsFactors = T)
library(cowplot)
library(ggstatsplot)
...
```{r figure5_final, echo=T, message=F, WARNING=F}
# Plotting Figure 5 as boxplot
...
```{r figure6_final, echo=T, message=F}
...
plot(data$d15N ~ data$d13C, data = data, pch = 16,col=barva,
     xlab = expression(paste(delta^13,"C"," (\u2030)")),
     ylab=expression(paste(delta^15,"N"," (\u2030)")), cex=1)
...
```{r figure7_final, echo=T, message=F}
