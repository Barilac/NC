...
# Plotting Figure 3 as boxplot
  ...
  boxplot(newdata[,2]~newdata[,4], xlab="", ylab=expression(paste(delta^15,"N"," (\u2030)")), col="white")
  
  stripchart(newdata[,2]~newdata[,4], method = "jitter",
             pch = 19, add = TRUE,
             col = c(org,bl,rd,gr), vertical = TRUE)
  ...
  boxplot(newdata[,1]~newdata[,4], xlab="", ylab=expression(paste(delta^13,"C"," (\u2030)")), col="white")
  
  stripchart(newdata[,1]~newdata[,4], method = "jitter",
             pch = 19, add = TRUE,
             col = c(org,bl,rd,gr), vertical = TRUE)
  ...
  # Function boxcount 
  boxcount=function (df, var, kat, k = 6){
    pos=1:length(levels(df[,kat]))
    nos=c()
    for (i in pos)
    {nos=append(nos, length(is.na(df[df[,kat]==levels(df[,kat])[i],var])==F))}
    ...
    for (j in pos)
      #{text(pos[j], mean(df[,var], na.rm=T), labels=paste("N: ", nos[j], sep=""))}
    {text(pos[j],min(df[,var], na.rm=T)-(max(df[,var],
    na.rm=T)-min(df[,var],
    na.rm=T))/k, labels=paste("n =  ", nos[j], sep=""), xpd=NA, cex=1.4)}
    list(nos, pos, mean(df[,var],na.rm=T))
  }
  ...
  png(height=6000, width=4500, res = 600, file=paste("Figure 4.png"))
  par(mar=c(8,5,5,2))
  par(cex.lab=1.2) # is for y-axis
  par(cex.axis=0.7) # size categories
  boxplot(newdata[,5]~newdata[,4], xlab="", ylab="P (ppm)", col="white")
  
  stripchart(newdata[,5]~newdata[,4], method = "jitter",
             pch = 19, add = TRUE,
             col = c(org,bl,rd,gr), vertical = TRUE)
...
png(height=3000, width=3500, res =600, file=paste("Figure 5.png"))
par(mar= c(5,5,1,1))
...
png(height=4500, width=8000, res = 600, file=paste("Figure 6.png"))
par(mfrow = c(1,2))
par(mar=c(8,5,5,2))
par(cex.lab=1.2) # is for y-axis
par(cex.axis=0.7) # size categories

boxplot(newdata[,3]~newdata[,4], xlab="", ylab="C_N_ratio", col="white")

stripchart(newdata[,3]~newdata[,4], method = "jitter",
           pch = 19, add = TRUE,
           col = c(org,bl,rd,gr), vertical = TRUE)
...
levels(newdata2$Category)
  ...
  boxplot(newdata2[,11]~newdata2[,8], xlab="", ylab=paste(namei), col="white")
  
  stripchart(newdata2[,11]~newdata2[,8], method = "jitter",
             pch = 19, add = TRUE,
             col = c(org,bl,rd,gr), vertical = TRUE)
...
png(filename="Figure 7.png", width = 2500, height = 2500, res=500)
