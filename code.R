...
plot(data$d15N ~ data$d13C, data=data, pch = shapes, col=barva, cex.lab=1.3, cex.axis = 1.3,
     xlab = expression(paste(delta^13,"C")), ylab=expression(paste(delta^15,"N")), cex=0.8, xlim=c(-33,-10), ylim=c(-6,15))
...
legend(-22, 1.8, legend=levels(data$Category2),
       text.col = c(org,blck, rd, gr,bl,yl,c3,c5,c6),bty = "n",
       cex=2.2, box.col = "white", ncol=4)
...
# Plotting Figure 4 as boxplot
  ...
  png(height=4500, width=8000, res = 600, file=paste("Figure 4.png"))
  par(mar=c(6,6,6,6))
  par(cex.lab=1.35) # is for y-axis
...
par(mar= c(5,5,1,1))
plot(data$d15N ~ data$d13C, data = data, pch = 16,col=barva, xlab = expression(paste(delta^13,"C")), ylab=expression(paste(delta^15,"N")), cex=1)
...
# Plotting Figure 7 as boxplot
for (i in 11)
{
  namei=colnames(newdata)[i]
  png(height=4500, width=8000, res = 600, file=paste("Figure 7.png",sep=""))
  par(mar=c(6,6,6,6))
  par(cex.lab=1.35) # is for y-axis
  
  par(cex.axis=1.35) # is for x-axis
  boxplot(newdata[,i]~newdata[,8], xlab="", ylab=paste(namei), col=barva2,
  )
  boxcount(newdata,i,8)
  
  pvaluei=round(kruskal.test(newdata[,i]~newdata[,8])[[3]], 3)
  ...
  dfi=round(kruskal.test(newdata[,i]~newdata[,8])[[2]], 3)
  
  mtext(3, line=2, text=paste("df:  ", dfi, sep=""),cex=1.35)
  
  chi=round(kruskal.test(newdata[,i]~newdata[,8])[[1]], 2)
