...
# reorder position of columns
# install.packages("dplyr")
library(dplyr)
newdata = newdata %>% relocate(d13C, .after=d15N)

newdata = newdata %>% relocate(P, .after=d15N)

newdata = newdata %>% relocate(C_N_ratio, .after=Category)

# order of figures 2,3,4,6
slou = c(1,2,3,5)

# Plotting Figures 2,3,4,6 as boxplots
for (i in slou)
{
  namei=colnames(newdata)[i]
  png(height=4500, width=8000, res = 600, file=paste("Figure ",i+1, ".png",sep=""))
  par(mar=c(6,6,6,10))
