...
# Plotting Figures 2,3,5,6 as boxplots
  for (i in c(1,2,4,5))
  {
    ...
    boxplot(newdata[,i]~newdata[,3], xlab="", ylab=paste(namei), col=barva2,
           )
    boxcount(newdata,i,3)
    
    pvaluei=round(kruskal.test(newdata[,i]~newdata[,3])[[3]], 3)
    ...
    dfi=round(kruskal.test(newdata[,i]~newdata[,3])[[2]], 3)
    
    mtext(3, line=2, text=paste("df:  ", dfi, sep=""),cex=1.35)
    
    chi=round(kruskal.test(newdata[,i]~newdata[,3])[[1]], 2)
