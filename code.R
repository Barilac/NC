...
#round percentage by C and N
rounded = round(newdata[,c(3,4)],4)

newdata = cbind(newdata, rounded)

newdata = newdata[-c(3,4)] # removed old data
# Drop levels
levels(newdata$Category)
levels(droplevels(newdata)$Category)
newdata$Category <- as.factor(as.character(newdata$Category))
...
data_select = newdata[,c(1,2,3,5,6,7)]
