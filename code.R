...
# Data were separated from the S1 file directly for this analysis.

# Loading data
raw = read.table(file = "data_dusiky8.txt", header=TRUE, sep="\t", dec=",", check.names = F, stringsAsFactors = T)
str(raw)
data = raw[,c(2,3,17)]
levels(raw[,17]) # levels of data - datasets, see Table 1
...
x <- seq(0, 20, 0.5)
y <- sin(x)
plot(x, y, type="l", col=c5)

barva = c(org,blck, rd, gr,bl,yl,c3,c5)
barva = barva[as.numeric(data$Category)]

# Plotting Figure 1

png(height=3000, width=3500, res = 500, file=paste("Figure1.png"))
...
legend(-17, 2, legend=levels(data$Category),
       text.col = c(org,blck, rd, gr,bl,yl,c3,c5),bty = "n",
...
######## ----- Figures 2 to 4 ---------
raw = read.table(file = "data_dusiky10.txt", header=TRUE, sep="\t", dec=",", check.names = F, stringsAsFactors = T)

newdata = raw[,c(2,3,8,9)]
...
# Plotting Figures 2 to 4 as boxplots
  for (i in 1:3)
  {
    namei=colnames(newdata)[i]
    png(height=4500, width=8000, res = 600, file=paste("23 12 29_", i, "_trebokov_categories.png"))
    par(mar=c(6,6,6,10))
...
barva = barva2[as.numeric(data$`archaeological category`)]


# Plotting Figure 5

png(height=3000, width=3500, res =600, file=paste("Figure5.png"))

plot(data$d15N ~ data$d13C, data = data, pch = 16,col=barva, xlab = "d13C", ylab="d15N", cex=1)

legend(-27.5, 2, legend=levels(data$`archaeological category`),
...
# ----- Figure 6 -------
raw = read.table(file = "PCA.txt", header=TRUE, sep="\t", dec=",", check.names = F, stringsAsFactors = T)


raw2 = raw[,c(1,2,3,4,5,7)]
data_normalized <- scale(raw2)
require(factoextra)
...
raw2 = raw[c(-3,-9),c(1,2,3,4,5,7)]
data = raw[c(-3,-9),]

data_normalized <- scale(raw2)


d = prcomp(data_normalized,scale=F, center=T) #scale = F, uz jsem scaloval clr
plot(d)
biplot(d)
...
barva = barva[as.numeric(data$`archaeological category`)]


# Plotting Figure 6

png(filename="Figure6.png", width = 2500, height = 2500, res=500)
...
legend(-4.5, -1.5, legend=levels(data$`archaeological category`),
